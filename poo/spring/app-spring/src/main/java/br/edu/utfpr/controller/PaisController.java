...
public class PaisController implements GenericController<Pais, PaisDTO, Integer> {
    ...
    public ResponseEntity<Object> create(@RequestBody PaisDTO entity) {
        if(this.paisService.existsBySigla(entity.getSigla())){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Conflito: A sigla informada já existe!! Favor verificar!");
        }
    ...
    public ResponseEntity<Object> getById(@PathVariable Integer id){
        ...
        if (pais.isEmpty()){
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Código de país não foi encontrado!!!");
        }
